shared_examples 'visa_mit_cit_framework_spec' do
  ...
  it 'should be able to persist and query mit transaction id' do
    properties = merge_properties(@properties, {'mit_cit_type' => 'CGEN',
  ...
  it 'should be able to do authorization with a matching mit_transaction_id and send the correct reference mit transaction id' do
    ...
    properties =  merge_properties(@properties, {'mit_cit_type' => 'MRSB',
                                                 'credential_on_file' => true,
                                                 'mit_ref_trx_id' => @kb_payment.transactions[0].id})
  ...
  it 'should be able to do successful authorization with mit ref id' do
    ...
    properties =  merge_properties(@properties, {'mit_cit_type' => 'MRSB',
                                                 'credential_on_file' => true,
                                                 'mit_ref_trx_id' => @kb_payment.transactions[0].id})
    payment_response = @plugin.authorize_payment(@pm.kb_account_id, @kb_payment.id, @kb_payment.transactions[1].id, @pm.kb_payment_method_id, @amount, @currency, properties, @call_context)
  ...
  it 'should not able to do successful authorization with non-matching mit ref id' do
    properties = merge_properties(@properties, {'mit_cit_type' => 'CGEN', 'credential_on_file' => true})
    @plugin.authorize_payment(@pm.kb_account_id, @kb_payment.id, @kb_payment.transactions[0].id, @pm.kb_payment_method_id, @amount, @currency, properties, @call_context)

    properties = merge_properties(@properties, {'mit_cit_type' => 'MRSB',
                                                        'credential_on_file' => true,
                                                        'mit_ref_trx_id' => 'test_id'})
    payment_response = @plugin.authorize_payment(@pm.kb_account_id, @kb_payment.id, @kb_payment.transactions[1].id, @pm.kb_payment_method_id, @amount, @currency, properties, @call_context)
  ...
  it 'should be able to do successful purchase with mit ref id' do
    ...
    properties =  merge_properties(@properties, {'mit_cit_type' => 'MRSB',
                                                         'credential_on_file' => true,
                                                         'mit_ref_trx_id' => @kb_payment.transactions[0].id})
    payment_response = @plugin.purchase_payment(@pm.kb_account_id, @kb_payment.id, @kb_payment.transactions[1].id, @pm.kb_payment_method_id, @amount, @currency, properties, @call_context)
    payment_response.status.should eq(:PROCESSED), payment_response.gateway_error
    payment_response.amount.should == @amount
